...
Python でなんらかのプロジェクトをはじめるうえで共通して使えそうなパッケージとかをまとめる（2021/09/02 現在）。選択肢ありすぎてよくわからんというのが調べた感想。

### Python バージョン、パッケージ（ついでにパッケージング）の管理

#### [Poetry](https://github.com/python-poetry/poetry)

最近来てる？パッケージ管理ツール。パッケージ管理と仮想環境の構築が簡単にできるので入れた方がよさそう。他の選択肢としては[Pipenv](https://github.com/pypa/pipenv)などがあるが、Poetry はさらにパッケージングまで行うことができるらしい。これらの情報はすべて`pyproject.toml`に記述されている。
...
個別のルール設定があまりできないらしい。ただ、個人的にはルール設定の自由度が高い個人の好みがでるだけなので、とくにマイナスにはならないと思ってる。

#### [mypy](http://www.mypy-lang.org/)

型チェックしてくれる。Python は 3.5 から型ヒントが導入されたけど、あくまでヒントであって型チェックはされない。ということでこいつの出番。
...
Python パッケージのドキュメントがよく同じようなフォーマットをしている正体。Docstring からドキュメントを自動生成してくれるのが非常に便利。
